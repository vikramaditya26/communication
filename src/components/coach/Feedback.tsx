    ...
    return (
      <div className="rounded-2xl border border-dashed border-line bg-paper-2 p-4">
        <div className="flex items-center gap-2 font-medium">
          ...
          {error.code === "no_key" ? "The AI coach is not switched on yet" : "Your Claude API key was not accepted"}
        </div>
        <p className="mt-1.5 text-sm leading-relaxed text-ink-2">
          Add your Claude API key as <code className="rounded bg-ink/5 px-1 py-0.5 text-[12px]">ANTHROPIC_API_KEY</code> in the <code className="rounded bg-ink/5 px-1 py-0.5 text-[12px]">.env.local</code> file (or in Vercel), then restart. Reading, listening and pronunciation marks still work without it.
        </p>
      </div>
    );
  }
